      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black mb-6 transform hover:scale-105 transition-all">
            <span className="text-gradient">Real People, Fake Results</span>
            <span className="text-3xl animate-bounce inline-block ml-4">🤥</span>
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Don't just take our word for it. Listen to these totally real, definitely not paid, 100% authentic testimonials!
            <span className="text-sm text-red-500 block mt-2 animate-pulse">*Results may vary based on your level of pagalpan</span>
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <Card key={index} className="group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 hover:rotate-1 bg-white/90 backdrop-blur-sm border-2 hover:border-pink-300">
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-center space-x-3">
                  <div className="text-3xl animate-bounce">{testimonial.image}</div>
                  <div className="text-center">
                    <div className="font-bold text-gray-800 group-hover:text-orange-600 transition-colors">{testimonial.name}</div>
        ...
        <div className="mt-12 text-center">
          <p className="text-sm text-gray-500 italic animate-bounce">
            *Results bilkul fake hain. Side effects include: excessive confidence, random flexing, aur logo ko protein chemistry explain karna without any knowledge.
          </p>
          <p className="text-xs text-red-500 mt-2 font-bold">
            Seriously though, please eat real protein. Don't become a meme like these people! 🤡
          </p>
        </div>
      </div>
    </section>
